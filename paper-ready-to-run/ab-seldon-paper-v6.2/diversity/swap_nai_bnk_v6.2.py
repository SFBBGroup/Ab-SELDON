...
def model_ab():
    ...
    #Checks if the new CDR has kept the same conformation as before the grafting
    if check_conformation == "yes":
        ...
        latest_ab = file.replace(".anarci", ".pdb")
        ...
        with redirect_stdout(open("pymol_conf_check.pml", "w", newline="")):
            ...
            print("align " + output_file.replace(".pdb", "") + ", " + latest_ab.replace(".pdb", ""))
    ...
    try:
        ...
        for rep in glob.glob("out-min.pdb"):
            ...
            cmd_bkp_error = "tar -cvf " + tested_canon[
                -1] + "_error.tar " + rep_chain + " " + rep + " " + rep_md + " " + rep_score + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(
                ".pdb", "_renum.pdb")
...
# whether or not the conformation of each new CDR will be compared to the original structure, and the RMSD threshold above
# which a CDR sequence will be discarded during a conformation check
...
for st in structures:
    ...
    if not error:
        ...
        final_num = str(final[-1]).replace("bs3", "bs4")
