...
def qual_ctrl():
    #checks the quality of the produced structure, looking at the CDR H3 RMSD (deviation estimated by IgFold)
        ...
        try:
            ...
            if new_score:
                ...
                if (float(av_new_score) < float(latest_score)) or (metropolis < approval_prob and float(av_new_score) != float(latest_score)):
                    ...
                    if float(av_new_score) > float(latest_score) and metropolis < approval_prob:
                        print(tested_canon[-1] + "|" + str(av_new_score) + "|Modification approved (metropolis)")
                        condition.append("PASS")
                    ...
                    rename_cmd = str("cp complex_" + latest_ab + " complex_" + new_file_name.replace(".anarci", ".pdb"))
            ...
            for rep in glob.glob("out-min.pdb"):
                ...
                cmd_del_int = "rm " + rep_chain + " " + rep + " " + rep_md + " " + rep_score + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(
                    ".pdb", "_renum.pdb")
                cmd_bkp_error = "tar -cvf " + tested_canon[-1] + "_error.tar " + rep_chain + " " + rep + " " + rep_md + " " + rep_score + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(".pdb","_renum.pdb")
                subprocess.run(cmd_bkp_error, shell=True, capture_output=True)
                subprocess.run(cmd_del_int, shell=True, capture_output=True)

            del tested_canon[-1]

#reading configuration file to determine how many swap cycles will be attempted, the mode of choice of CDR H3 for grafting (random or probabilistic),
#and RMSD threshold above which a modelled H3 is deemed "low quality" and discarded
...
for st in structures:
    ...
    if not "ERROR" in error:
        ...
        final_num = str(final[-1]).replace("bs1", "bs2")
