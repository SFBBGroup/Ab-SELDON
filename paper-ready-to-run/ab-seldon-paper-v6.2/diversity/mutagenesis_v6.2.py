...
for st in structures:
    ...
    tested_canon = []
    #stores the IDs of mutations that were rejected for producing CDRs with a different conformation compared to the original structure
    rejected = []
    ...
    while len(tested_canon) < n_cycles:
        ...
        for new in latest:
            for file in glob.glob(new):
                ...
                id = file.split("_")[2]
                # list of chains and CDR numbers. All CDRs will be subjected to mutagenesis
                Ch_list = ["H", "L", "H3"]
                HL_list = ["1", "2", "3"]
    ...
    if not error:
        ...
        final_num = str(final[-1]).replace("bs4", "bs5")
