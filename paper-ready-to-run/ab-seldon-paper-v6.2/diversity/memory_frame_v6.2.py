...
def model_hyb_l():
    ...
    for key, value in hybrids_l.items():
        ...
        output_file = (fasta.replace(fasta.split("_")[-1], "") + "hyb-L_" + number + ".pdb").replace("bs5_", "")
                ...
                for seq_li in open_light_fasta:
                    if seq_li.startswith(">") and "L" in seq_li:
                        ...
                        full_mem = "fullmem_" + h_id[0] + h_id[1].replace(".fasta", "") + "_" + l_id[0] + l_id[
                            1].replace(
                            ".fasta", "")
                        full_mem_fasta = (fasta.replace(fasta.split("_")[-1], full_mem)).replace("bs5_", "") + ".pdb"
...
def pymol_aln():
    ...
    log_pml_prt = open("pymol_out_" + fasta.replace(".fasta", ".log").replace("bs5_", ""), "a", newline="")
...
def scoring():
    ...
    complex_chain_lengths = [pml_out.split("Selector: selection \"new_h")[-1].split(" ")[3],
                             pml_out.split("Selector: selection \"new_l")[-1].split(" ")[3],
                             pml_out.split("Selector: selection \"new_complex")[-1].split(" ")[3]]
    global ab_chains_resn
    ab_chains_resn = str(
        int(complex_chain_lengths[2]) - int(complex_chain_lengths[0]) - int(complex_chain_lengths[1]) + 1) + "-" + str(
        complex_chain_lengths[-1])
    ...
    try:
        ...
        cmd_renum = "pdb_fixinsert " + latest_ab_complex + " > " + latest_ab_complex.replace(".pdb", "_renum.pdb")
        subprocess.run(cmd_renum, shell=True, capture_output=True)
        # Protonation adjustment with propka
        cmd_prot = "pdb2pqr30 --with-ph " + ph + " --ff AMBER --ffout AMBER --pdb-output out_pdb2pqr.pdb --titration-state-method propka " + latest_ab_complex.replace(
            ".pdb", "_renum.pdb") + " out_pdb2pqr.pqr"
        # pdb4amber
        ...
        if new_score:
            ...
            new_file_name = low.replace("bs5_", "")
            ...
            if (float(av_new_score) < float(latest_score)) or (metropolis < approval_prob and float(av_new_score) != float(latest_score)):
                if (float(av_new_score) < float(latest_score)):
                    print(new_file_name + "|" + str(float(av_new_score)) + "|Modification approved")
                if (float(av_new_score) > float(latest_score)) and metropolis < approval_prob:
                    print(new_file_name + "|" + str(float(av_new_score)) + "|Modification approved (metropolis)")
                with redirect_stdout(updt_log):
                    print("\t" + new_file_name + "|" + str(float(av_new_score)) + "|PASS")
            if (float(av_new_score) > float(latest_score) and metropolis > approval_prob) or float(av_new_score) == float(latest_score):
            ...
            for rep in glob.glob("out-min.pdb"):
                ...
                rep_chain = "CHN_" + rep
                rep_score = "SCO_" + rep
                cmd_del_int = "rm " + rep_chain + " " + rep + " " + rep_md + " " + rep_score + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(".pdb","_renum.pdb")
        ...
        for rep in glob.glob("out-min.pdb"):
            ...
            rep_chain = "CHN_" + rep
            rep_score = "SCO_" + rep
            cmd_del_int = "rm " + rep_chain + " " + rep + " " + rep_md + " " + rep_score + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(
                ".pdb", "_renum.pdb")
            cmd_bkp_error = "tar -cvf " + low + "_error.tar " + rep_chain + " " + rep + " " + rep_md + " " + rep_score + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(
                ".pdb", "_renum.pdb")
...
for struc in glob.glob("bs5_*.anarci"):
...
for st in structures:
    ...
    for fasta in glob.glob(orig):
                ...
                if header.startswith(">"):
                    for id in best_h[:hyb]:
                        if id in header:
                            ...
                            #take the memory sequence selected and swap its CDRs with those of the original antibody
                            mem_nai_swap()
                            #store sequence and header information for both the memory and the naive (original) chains
        ...
        pml_rmsd_h = {}
        pml_rmsd_l = {}
        open_pyml = open("pymol_out_" + fasta.replace(".fasta", ".log").replace("bs5_", ""), "r").readlines()
        ...
        pml_rmsd_fmem = {}
        open_pyml2 = open("pymol_out_" + fasta.replace(".fasta", ".log").replace("bs5_", ""), "r").readlines()
        ...
        for sc in open_final_log:
            if sc.startswith("bs5_"):
                name = (sc.split("|")[0]).replace("\n", "")
                progress = (name.split("_")[-1]).replace(".anarci", "")
                new_progress = '{:06}'.format(int((name.split("_")[-1]).replace(".anarci", "")) + 1)
                run = (name.split(progress)[0]).replace("bs5","FINAL")
                run_list.append(run)
        ...
        with redirect_stdout(write_final_log):
            ...
            if any_approval == "FAIL":
                print("#NOTE:Framework maturation failed to produce better structures. FINAL structure is the same as the bs5 structure.")
        ...
        rename_cmd2 = str("cp " + best.replace(".anarci", ".fasta") + " " + run_list[-1] + str(num_list[-1]) + ".fasta")
