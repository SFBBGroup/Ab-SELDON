...
def scoring():
    ...
    # Alignment
    latest_ab = filename + ".pdb"
    latest_complex = "complex_" + input_name + ".pdb"
    ...
    complex_chain_lengths = [pml_out.split("Selector: selection \"new_h")[-1].split(" ")[3], pml_out.split("Selector: selection \"new_l")[-1].split(" ")[3], pml_out.split("Selector: selection \"new_complex")[-1].split(" ")[3]]
    global ab_chains_resn
    ab_chains_resn = str(int(complex_chain_lengths[2]) - int(complex_chain_lengths[0]) - int(complex_chain_lengths[1]) + 1) + "-" + str(complex_chain_lengths[-1])
    ...
    for rep in glob.glob("out-min.pdb"):
        rep_chain = "CHN_" + rep
        rep_score = "SCO_" + rep
        cmd_del_int = "rm " + rep_chain + " " + rep + " " + rep_score + " " + " min.out *heat*.out *.info *.rst7 *.crd *.leap *.pqr *.parm7 *.mdinfo *.mdout *.nc " + latest_ab_complex.replace(
            ".pdb", "_renum.pdb")
        subprocess.run(cmd_del_int, shell=True, capture_output=True)

#start of the script
input_name = "cfg:prepare|input_complex"
...
for setting in cfg:
    ...
    if "prepare|input_complex" in setting:
        input_name = str(setting.split("=")[1].replace("\n", "")).replace("complex_","").replace(".pdb","")
...
for amount in range(1,(int(num_struc) + 1)):
    ...
    number = next(counter)
    filename = "bs1_" + number + "_" + input_name + "_" + code_name + "_000001"
